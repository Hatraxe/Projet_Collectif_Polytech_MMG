...
def generate_graph_age(request):
    ...
    df['Age'] = df['Date de naissance'].apply(
    ...
    wedges, texts, autotexts = plt.pie(age_group_counts, labels=age_group_counts.index, autopct='%1.1f%%',
                                       startangle=90)
    plt.title('Âge des patients', pad=30)
    plt.axis('equal')

    # Place the legend outside the pie chart
    plt.legend(wedges, age_group_counts.index, title="Tranches d'âge", loc="center left", bbox_to_anchor=(1, 0, 0.5, 1))
...
def generate_graph_cree_par(request):
    ...
    # FR: Faire un graphique
    plt.figure(figsize=(10, 8))
    wedges, texts, autotexts = plt.pie(cree_counts, labels=cree_counts.index, autopct='%1.1f%%', startangle=90)
    plt.title('Créé par', pad=30)
    plt.axis('equal')

    # Place the legend outside the pie chart
    plt.legend(wedges, cree_counts.index, title="Créé par", loc="center left", bbox_to_anchor=(1, 0, 0.5, 1))
...
def generate_indicator_breakdown_of_times_workday(request):
    ...
    for month, group in grouped:
        ...
        row = [str(month)]
        for key in all_hours:  # Ensure we only use the hours in all_hours
            value = times[key]
            if total != 0:
        ...
        if len(row) == len(table.field_names):
            ...
            raise ValueError(f"Row has incorrect number of values, {len(row)} != {len(table.field_names)}")
